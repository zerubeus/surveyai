...
def export_zip(db: SupabaseDB, task_id: str, payload: dict[str, Any]) -> None:
    """
    ...
    Payload: {"report_id": str, "dataset_id": str}
    """
    import zipfile
    ...
    logger.info("export_zip_start", report_id=report_id)
    ...
    safe_name = re.sub(r"[^a-zA-Z0-9_\-]", "_", project_name.replace(" ", "_"))[:40]
    import re as _re
    safe_name = _re.sub(r"[^a-zA-Z0-9_\-]", "_", project_name.replace(" ", "_"))[:40]
    year = datetime.now(timezone.utc).year

    sections = db.select("report_sections", filters={"report_id": report_id})
    sections.sort(key=lambda s: s.get("sort_order", 0))

    # Step 2: Generate DOCX + PDF
    db.update_task_progress(task_id, 15, "Generating DOCX...")
    chart_images = _load_chart_images(db, sections)
    docx_bytes = _generate_docx(report, project_name, sections, chart_images)

    db.update_task_progress(task_id, 30, "Generating PDF...")
    pdf_bytes = _generate_pdf(report, project_name, sections, chart_images)

    # Step 3: Load ALL chart images for this project
    db.update_task_progress(task_id, 40, "Loading chart images...")
    all_charts = db.select("charts", filters={"project_id": report["project_id"]})
    chart_png_files: list[tuple[str, bytes]] = []
    for chart in all_charts:
        if chart.get("file_path"):
            try:
                img_bytes = db.download_file("charts", chart["file_path"])
                safe_title = _re.sub(r"[^a-zA-Z0-9_\-]", "_", (chart.get("title") or "chart")[:40])
                fname = f"charts/{safe_title}_{chart['id'][:8]}.png"
                chart_png_files.append((fname, img_bytes))
            except Exception as e:
                logger.warning("chart_download_failed", chart_id=chart["id"], error=str(e))

    # Step 4: Load cleaned dataset CSV
    db.update_task_progress(task_id, 55, "Loading cleaned dataset...")
    csv_bytes: bytes | None = None
    if dataset_id:
        try:
            dataset_rows = db.select("datasets", filters={"id": dataset_id})
            if dataset_rows:
                dataset = dataset_rows[0]
                file_path = dataset.get("original_file_path", "")
                raw = db.download_file("uploads", file_path)
                buf = io.BytesIO(raw)
            ...
            logger.warning("dataset_export_failed", error=str(e))

    # Step 5: Bundle into ZIP
    db.update_task_progress(task_id, 70, "Bundling ZIP...")
    zip_buf = io.BytesIO()
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr(f"{safe_name}_Report_{year}.docx", docx_bytes)
        zf.writestr(f"{safe_name}_Report_{year}.pdf", pdf_bytes)
        for fname, img in chart_png_files:
            zf.writestr(fname, img)
        if csv_bytes:
            zf.writestr(f"{safe_name}_cleaned_data_{year}.csv", csv_bytes)
        # Add a README
        readme = f"""Chisquare Analysis Bundle — {project_name}
Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d')}

Contents:
- {safe_name}_Report_{year}.docx: Full report in Word format
- {safe_name}_Report_{year}.pdf: Full report in PDF format
- charts/: All visualisation charts as PNG images ({len(chart_png_files)} charts)
{"- " + safe_name + "_cleaned_data_" + str(year) + ".csv: Cleaned dataset" if csv_bytes else ""}

Generated by Chisquare (chisquare.net)
"""
    ...
    # Step 6: Upload ZIP
    db.update_task_progress(task_id, 85, "Uploading ZIP...")
    zip_path = f"{created_by}/{report['project_id']}/{safe_name}_{year}_bundle.zip"
    db.upload_file("reports", zip_path, zip_bytes, "application/zip")
    signed_url = db.get_signed_url("reports", zip_path, expires_in=604800)
    ...
    db.update("reports", {"status": "exported"}, {"id": report_id})
    db.complete_task(task_id, {
        "message": "ZIP bundle exported successfully",
        "report_id": report_id,
        "zip_signed_url": signed_url,
        "charts_included": len(chart_png_files),
        "includes_dataset": csv_bytes is not None,
    })
    logger.info("zip_exported", path=zip_path, size=len(zip_bytes), charts=len(chart_png_files))
