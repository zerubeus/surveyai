...
def _load_dataframe_robust(file_bytes: bytes, file_type: str) -> pd.DataFrame:
    """Load dataset from bytes with robust handling for all formats."""
    if file_type in (
        "xlsx", "xls",
...
def apply_cleaning_operation(
    ...
    df = _load_dataframe_robust(file_bytes, file_type)
